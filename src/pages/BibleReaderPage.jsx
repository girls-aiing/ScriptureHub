...
const HYMNS_SACRED_SONGS = [
  ...
  { num:26, title:'Grace Greater Than Our Sin', author:'Julia H. Johnston (1911)', category:'Grace', key:'Eb',
    verses:[
      ...
      <div style={sm.panel}>
        <div style={sm.topBar}>
          ...
          <div style={sm.topRight}>
            <button style={sm.listenBtn} onClick={()=>isReading ? onStop() : onSpeak(verse.text)}>
        ...
        <div style={sm.dots}>
          {hymn.verses.map((_,i)=>(
            <button key={i}
              style={{...sm.dot, background: i===verseIdx ? '#f0c040' : 'rgba(240,192,64,0.25)'}}
        ...
        <div style={sm.navRow}>
          <button style={{...sm.navBtn, opacity: verseIdx===0 ? 0.3 : 1}}
            disabled={verseIdx===0}
            onClick={()=>{ onStop(); setVerseIdx(i=>i-1) }}>← Prev</button>
          ...
          <button style={{...sm.navBtn, opacity: verseIdx===hymn.verses.length-1 ? 0.3 : 1}}
            ...
            onClick={()=>{ onStop(); setVerseIdx(i=>i+1) }}>Next →</button>
        </div>
      </div>
    </div>
  )
...
export default function BibleReaderPage() {
  ...
  // ── Karaoke state ─────────────────────────────────────────────
  const [karaokeWord,  setKaraokeWord]  = useState(null) // {verseNum, wordIdx}
  const karaokeMap = useRef([])                          // [{verseNum,wordIdx,charStart,charEnd}]

  // ── Hymnal state ──────────────────────────────────────────────
  const [hymnCollection, setHymnCollection] = useState('ancient')
  ...
  // ── VoiceGuide: view changes ──────────────────────────────────
  useEffect(()=>{
    if (view === 'hymns')       announceHymnView('hymns')
    if (view === 'hymn-detail') announceHymnView('hymn-detail')
    if (view === 'books' && prevViewRef.current !== null) announceHymnView('books')
    prevViewRef.current = view
  },[view])

  // ── VoiceGuide: singing mode ──────────────────────────────────
  useEffect(()=>{
  ...
  // Build word-position map for a single verse
  function buildSingleMap(text, verseNum) {
    const cleaned = cleanText(text)
    const map = []
    let pos = 0
    cleaned.split(/\s+/).forEach((word, wi) => {
      if (!word) return
      const start = cleaned.indexOf(word, pos)
      map.push({ verseNum, wordIdx: wi, charStart: start, charEnd: start + word.length })
      pos = start + word.length
    })
    return { cleaned, map }
  }

  // Build word-position map across ALL verses (for speakAll)
  function buildFullMap(versesArr) {
    const parts = []
    const map   = []
    let offset  = 0
    versesArr.forEach(v => {
      ...
      cleaned.split(/\s+/).forEach((word, wi) => {
        if (!word) return
        const localStart = cleaned.indexOf(word, pos)
        map.push({
          verseNum:  v.verse,
          wordIdx:   wi,
          charStart: offset + localStart,
          charEnd:   offset + localStart + word.length,
        })
        pos = localStart + word.length
      })
      parts.push(cleaned)
      offset += cleaned.length + 2  // +2 for the '  ' separator
    })
    return { joined: parts.join('  '), map }
  }

  // speak() — single verse (from verse context menu)
  const speak = (text, vNum = null) => {
    stopNarration()
    const { cleaned, map } = buildSingleMap(text, vNum)
    karaokeMap.current = map
    const u = new SpeechSynthesisUtterance(cleaned)
    u.rate = 0.88
    u.onstart    = () => { setIsReading(true);  setReadingVerse(vNum) }
    u.onend      = () => { setIsReading(false); setReadingVerse(null); setKaraokeWord(null) }
    u.onerror    = () => { setIsReading(false); setReadingVerse(null); setKaraokeWord(null) }
    u.onboundary = (e) => {
      if (e.name !== 'word') return
      const hit = karaokeMap.current.find(w => e.charIndex >= w.charStart && e.charIndex < w.charEnd)
      if (hit) setKaraokeWord({ verseNum: hit.verseNum, wordIdx: hit.wordIdx })
    }
    window.speechSynthesis.speak(u)
  }

  // speakAll() — full chapter with karaoke across all verses
  const speakAll = () => {
    stopNarration()
    if (!verses.length) return
    const { joined, map } = buildFullMap(verses)
    karaokeMap.current = map
    const u = new SpeechSynthesisUtterance(joined)
    u.rate = 0.88
    u.onstart    = () => { setIsReading(true);  setReadingVerse('all') }
    u.onend      = () => { setIsReading(false); setReadingVerse(null); setKaraokeWord(null) }
    u.onerror    = () => { setIsReading(false); setReadingVerse(null); setKaraokeWord(null) }
    u.onboundary = (e) => {
      if (e.name !== 'word') return
  ...
  const collectionLabel  = hymnCollection==='ancient' ? 'Hymns Ancient & Modern' : 'Sacred Songs & Solos'
    ...
    return (
      <div style={s.page}>
        ...
        <div style={hm.collectionHeader}>
          ...
          <div style={hm.collectionSub}>Complete edition · {collectionTotal} hymns total</div>
        </div>
        <div style={s.searchWrap}>
          <span style={s.searchIcon}>🔍</span>
          <input style={s.searchInput} type="text"
            placeholder="Find a hymn by number, title, author, or lyric…"
            value={hymnSearch} onChange={e=>setHymnSearch(e.target.value)} />
        ...
        <div style={s.catRow}>
          {activeCategories.map(c=>(
            <button key={c}
              ...
              onClick={()=>setHymnCategory(c)}>{c}</button>
          ))}
        </div>
        <div style={s.ctrlRow}>
          <span style={s.resultCount}>{filteredHymns.length} of {activeCollection.length} hymns shown</span>
      ...
      <header style={s.readerHeader}>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:'8px',alignItems:'flex-end'}}>
          <button style={{...s.listenBtn,opacity:(loading||!verses.length)?0.35:1}}
            ...
            onClick={()=>isReading?stopNarration():speakAll()}>
            {isReading?'⏹ Stop':'🔊 Listen'}
          </button>
        ...
        {!loading&&!error&&verses.length>0&&(
          <>
            ...
            {displayVerses.map(v=>{
              ...
              const words     = cleanText(v.text).split(/\s+/)
              return (
                <div key={v.verse} style={{position:'relative'}}>
                  <p style={{
                    ...
                    {words.map((word, wi) => {
                      const lit = karaokeWord &&
                        karaokeWord.verseNum === v.verse &&
                        karaokeWord.wordIdx  === wi
                      return (
                        <span key={wi} style={lit ? s.karaokeWord : s.karaokeIdle}>
                          {word}{' '}
                        </span>
                      )
                    })}
...
const sm = {
  ...
  topBar:     { display:'flex', justifyContent:'space-between', alignItems:'flex-start', flexWrap:'wrap', gap:'1rem' },
  ...
  listenBtn:  { padding:'8px 18px', borderRadius:'20px', border:'1px solid #f0c040',
                color:'#f0c040', background:'transparent', cursor:'pointer', fontFamily:'inherit', fontSize:'0.9rem' },
  closeBtn:   { padding:'8px 18px', borderRadius:'20px', border:'1px solid #c8a96e',
                color:'#c8a96e', background:'transparent', cursor:'pointer', fontFamily:'inherit', fontSize:'0.9rem' },
  ...
  lyricsBox:  { flex:1, display:'flex', alignItems:'center', justifyContent:'center',
                ...
                border:'1px solid rgba(240,192,64,0.15)', padding:'2rem', minHeight:'260px' },
  lyrics:     { whiteSpace:'pre-wrap', fontSize:'1.9rem', color:'#f0e6d2',
                fontFamily:"'Crimson Text',serif", lineHeight:'2.2', textAlign:'center', margin:0 },
  dots:       { display:'flex', gap:'10px', justifyContent:'center', flexWrap:'wrap' },
  dot:        { width:'12px', height:'12px', borderRadius:'50%', border:'none', cursor:'pointer', transition:'background 0.2s' },
  ...
  navBtn:     { padding:'10px 24px', borderRadius:'25px', border:'1px solid #f0c040',
                color:'#f0c040', background:'transparent', cursor:'pointer', fontFamily:'inherit', fontSize:'1rem' },
  navCount:   { color:'#c8a96e', fontSize:'0.95rem', fontStyle:'italic' },
}

// ── Hymn Centre Styles ────────────────────────────────────────────
const hm = {
  welcomeBanner:   { textAlign:'center', maxWidth:'700px', margin:'0 auto 2rem' },
  ...
  welcomeSub:      { color:'#c8a96e', fontSize:'1rem', lineHeight:'1.7', fontStyle:'italic', margin:'0 0 2rem' },
  switcherRow:     { display:'flex', gap:'1rem', justifyContent:'center', flexWrap:'wrap', maxWidth:'800px', margin:'0 auto 2rem' },
  switchBtn:       { flex:'1', minWidth:'240px', maxWidth:'340px', padding:'1.5rem 1.25rem',
                     borderRadius:'16px', border:'1px solid rgba(240,192,64,0.25)',
  ...
  numBadge:        { background:'rgba(240,192,64,0.12)', color:'#f0c040',
                     borderRadius:'8px', padding:'2px 8px', fontSize:'0.75rem', fontWeight:'700' },
...
const s = {
  ...
  quickBar:        { display:'flex', justifyContent:'center', gap:'1rem', marginBottom:'1.5rem', flexWrap:'wrap' },
  quickBtn:        { padding:'8px 18px', borderRadius:'20px', border:'1px solid rgba(240,192,64,0.4)',
                     background:'rgba(240,192,64,0.07)', color:'#f0c040', cursor:'pointer',
                     fontFamily:'inherit', fontSize:'0.95rem', display:'flex', alignItems:'center', gap:'6px' },
  ...
  tab:             { padding:'10px 22px', borderRadius:'20px', border:'1px solid transparent',
                     background:'transparent', color:'#f0c040', cursor:'pointer', fontFamily:'inherit', fontSize:'1rem' },
  ...
  chapterBtn:      { background:'rgba(255,255,255,0.04)', border:'1px solid rgba(240,192,64,0.4)',
                     color:'#f0c040', borderRadius:'50%', width:'54px', height:'54px',
                     display:'flex', alignItems:'center', justifyContent:'center',
  ...
  iconBtn:         { background:'rgba(240,192,64,0.08)', border:'1px solid rgba(240,192,64,0.3)',
                     color:'#f0c040', borderRadius:'20px', padding:'5px 12px', cursor:'pointer',
                     fontFamily:'inherit', fontSize:'0.9rem', display:'flex', alignItems:'center', gap:'4px' },
  scriptureColumn: { maxWidth:'800px', margin:'0 auto' },
  versionBadge:    { textAlign:'center', color:'#c8a96e', fontSize:'0.95rem', fontStyle:'italic',
  ...
  verse:           { marginBottom:'1.4rem', cursor:'pointer', padding:'8px 10px',
                     borderRadius:'8px', transition:'background 0.2s', lineHeight:'1.8', userSelect:'text' },
  verseActive:     { background:'rgba(240,192,64,0.06)', borderLeft:'3px solid #f0c040', paddingLeft:'14px' },
  ...
  // ── Karaoke ──
  karaokeWord:     { background:'rgba(240,192,64,0.32)', color:'#fff',
                     borderRadius:'3px', padding:'0 2px',
                     boxShadow:'0 0 10px rgba(240,192,64,0.6)',
                     transition:'background 0.08s',
                     display:'inline', fontSize:'1.4rem' },
  karaokeIdle:     { display:'inline', fontSize:'1.4rem', transition:'background 0.12s' },
  centerMsg:       { textAlign:'center', fontSize:'1.15rem', color:'#f0c040', marginTop:'4rem', fontStyle:'italic' },
  spinner:         { width:'40px', height:'40px', border:'3px solid rgba(240,192,64,0.2)',
  ...
  errorBox:        { textAlign:'center', color:'#e67e22', padding:'2.5rem 2rem',
                     border:'1px solid #e67e22', borderRadius:'14px', maxWidth:'480px', margin:'3rem auto' },
  ...
  vMenu:           { position:'absolute', left:'0', top:'100%', zIndex:150,
                     ...
                     boxShadow:'0 8px 32px rgba(0,0,0,0.65)', animation:'fadeUp 0.15s ease' },
  mBtn:            { display:'block', width:'100%', textAlign:'left', padding:'9px 14px',
                     background:'transparent', border:'none', color:'#f0e6d2',
                     cursor:'pointer', fontFamily:'inherit', fontSize:'0.95rem', borderRadius:'6px' },
  ...
  shareLink:       { display:'block', padding:'9px 14px', color:'#f0e6d2',
                     textDecoration:'none', fontFamily:'inherit', fontSize:'0.95rem', borderRadius:'6px' },
  toast:           { position:'fixed', bottom:'28px', left:'50%', transform:'translateX(-50%)',
                     background:'#2a1a08', border:'1px solid #f0c040', color:'#f0e6d2',
                     padding:'12px 26px', borderRadius:'25px', zIndex:999, fontSize:'1rem',
                     boxShadow:'0 4px 20px rgba(0,0,0,0.5)', animation:'fadeUp 0.2s ease', whiteSpace:'nowrap' },
  overlay:         { position:'fixed', inset:0, background:'rgba(0,0,0,0.78)', zIndex:400,
  ...
  noteTA:          { width:'100%', background:'#120a05', border:'1px solid rgba(240,192,64,0.4)',
                     color:'#f0e6d2', borderRadius:'8px', padding:'12px', fontFamily:'inherit',
                     fontSize:'1.1rem', lineHeight:'1.7', resize:'vertical', boxSizing:'border-box' },
  ...
  modalCancel:     { padding:'10px 22px', borderRadius:'25px', border:'1px solid #888',
  ...
  searchInput:     { width:'100%', padding:'12px 40px 12px 42px', borderRadius:'30px',
                     border:'1px solid rgba(240,192,64,0.5)', background:'rgba(255,255,255,0.05)',
                     color:'#f0e6d2', fontFamily:"'Crimson Text',serif", fontSize:'1.05rem',
                     outline:'none', boxSizing:'border-box' },
  searchClear:     { position:'absolute', right:'14px', background:'none', border:'none',
                     color:'#c8a96e', cursor:'pointer', fontSize:'1rem' },
  catRow:          { display:'flex', flexWrap:'wrap', gap:'8px', justifyContent:'center', marginBottom:'1.25rem' },
  catPill:         { padding:'6px 16px', borderRadius:'20px', border:'1px solid rgba(240,192,64,0.35)',
                     background:'transparent', color:'#c8a96e', cursor:'pointer',
  ...
  ctrlRow:         { display:'flex', justifyContent:'space-between', alignItems:'center',
                     maxWidth:'960px', margin:'0 auto 1.25rem', flexWrap:'wrap', gap:'0.5rem', padding:'0 0.5rem' },
  resultCount:     { color:'#c8a96e', fontStyle:'italic', fontSize:'0.95rem' },
  sortBtn:         { padding:'5px 14px', borderRadius:'15px', border:'1px solid rgba(240,192,64,0.3)',
                     background:'transparent', color:'#c8a96e', cursor:'pointer',
                     fontFamily:'inherit', fontSize:'0.85rem' },
  sortBtnOn:       { border:'1px solid #f0c040', color:'#f0c040', background:'rgba(240,192,64,0.1)' },
  ...
  metaBadge:       { fontSize:'0.83rem', padding:'4px 12px', borderRadius:'15px',
                     background:'rgba(240,192,64,0.1)', color:'#f0c040',
                     border:'1px solid rgba(240,192,64,0.3)' },
  divider:         { textAlign:'center', color:'#f0c040', fontSize:'1.5rem', margin:'2rem 0', letterSpacing:'1rem' },
  hymnVerseList:   { display:'flex', flexDirection:'column', gap:'2rem' },
  hymnVerseBlock:  { position:'relative', padding:'1.4rem 1.6rem', background:'rgba(255,255,255,0.03)',
                     borderRadius:'12px', border:'1px solid rgba(240,192,64,0.15)',
  ...
  singBtn:         { position:'absolute', top:'12px', right:'12px', background:'transparent',
                     ...
                     display:'flex', alignItems:'center', justifyContent:'center', color:'#f0c040' },
}