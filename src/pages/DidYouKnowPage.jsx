...
const ALL_SECRETS = [
  ...
  { id: 15, category: 'Prophecy', emoji: '🔮', title: 'The 70 Weeks of Daniel', fact: 'Daniel 9 predicted the exact number of years between the decree to rebuild Jerusalem and the arrival of the Messiah — 483 years. This calculation, using the Jewish calendar, lands precisely on the year Jesus entered Jerusalem on Palm Sunday.', verse: 'Daniel 9:25', verseText: 'Know and understand this: From the time the word goes out to restore and rebuild Jerusalem until the Anointed One, the ruler, comes, there will be seven "sevens," and sixty-two "sevens."', deepDive: 'Sir Robert Anderson\'s landmark book "The Coming Prince" (1894) demonstrated mathematically that Daniel\'s 69 weeks of years (483 years) ended on April 6, 32 AD — the exact date of the Triumphal Entry.' },
  ...
  { id: 19, category: 'Science & Bible', emoji: '🔬', title: 'The Stars Are Innumerable', fact: 'Jeremiah 33:22 says the stars cannot be counted. For centuries, people thought there were only about 1,100 visible stars. Today we know there are an estimated 10 sextillion stars — 10,000,000,000,000,000,000,000.', verse: 'Jeremiah 33:22', verseText: 'I will make the descendants of David my servant and the Levites who minister before me as countless as the stars in the sky and as measureless as the sand on the seashore.', deepDive: 'Hipparchus catalogued 1,080 stars in 129 BC. Ptolemy counted 1,022. The Bible\'s claim of uncountable stars was considered an exaggeration until modern telescopes revealed the true scale of the universe.' },
  ...
  { id: 30, category: 'Jesus & Gospels', emoji: '✝️', title: 'The Triumphal Entry Date', fact: 'Jesus entered Jerusalem on what we call Palm Sunday, exactly 483 years (69 "weeks" of years) after the decree of Artaxerxes to rebuild Jerusalem in 444 BC — fulfilling Daniel\'s prophecy to the exact day.', verse: 'Zechariah 9:9', verseText: 'Rejoice greatly, Daughter Zion! Shout, Daughter Jerusalem! See, your king comes to you, righteous and victorious, lowly and riding on a donkey.', deepDive: 'Using the Jewish 360-day prophetic calendar, 69 weeks of years = 173,880 days. From March 5, 444 BC to April 6, 32 AD is exactly 173,880 days. The precision is breathtaking.' },
  ...
  { id: 49, category: 'Biblical Characters', emoji: '👑', title: 'Mary Magdalene — First Witness', fact: 'Mary Magdalene was the first person to see the risen Jesus (John 20:14-16). In 1st century Jewish culture, women\'s testimony was not accepted in court. The fact that the Gospels record a woman as the first witness argues for authenticity — no one inventing the story would have chosen a woman.', verse: 'John 20:16', verseText: 'Jesus said to her, "Mary." She turned toward him and cried out in Aramaic, "Rabboni!" (which means "Teacher").', deepDive: 'Scholars call this the "criterion of embarrassment" — details too culturally awkward to invent are likely historically true. Choosing Mary as the first witness would have been embarrassing in that culture, making it almost certainly authentic.' },
...
export default function DidYouKnowPage() {
  const [viewed, setViewed] = useState(() => {
    const saved = localStorage.getItem('scriptureHub_viewed')
    return saved ? JSON.parse(saved) : []
  })
  const [bookmarks, setBookmarks] = useState(() => {
    const saved = localStorage.getItem('scriptureHub_bookmarks')
    return saved ? JSON.parse(saved) : []
  })
  ...
  const categoryColor = currentSecret ? (CATEGORY_COLORS[currentSecret.category] || '#f0c040') : '#f0c040'
    ...
    <div style={{
      ...
      <div style={{ textAlign: 'center', marginBottom: '1.5rem' }}>
        ...
        <p style={{ color: '#c8a96e', fontSize: '0.95rem', margin: 0 }}>
          Daily Manna — Biblical secrets you've never heard before ✝️
        </p>
      </div>

      <div style={{
        maxWidth: '780px', margin: '0 auto 1.5rem auto',
        ...
        <div>
          <div style={{ color: '#f0c040', fontWeight: 'bold', fontSize: '1.1rem' }}>
            🔓 {totalViewed} / {totalSecrets} Secrets Discovered
          </div>
          <div style={{ color: '#c8a96e', fontSize: '0.8rem', marginTop: '0.25rem' }}>
            {totalSecrets - totalViewed} secrets still waiting for you...
          </div>
        </div>
      ...
      <div style={{
        ...
        {[
          ...
          <button key={key} onClick={() => setTab(key)} style={{
            ...
            background: tab === key ? 'linear-gradient(135deg, #f0c040, #c8860a)' : 'transparent',
            ...
            <div style={{
              ...
              {CATEGORIES.map(cat => (
                <button key={cat} onClick={() => setActiveCategory(cat)} style={{
                  padding: '0.35rem 0.85rem', borderRadius: '16px', fontSize: '0.78rem',
                  border: activeCategory === cat ? 'none' : '1px solid rgba(240,192,64,0.2)',
            ...
            {currentSecret && (
              <div style={{
                ...
                <div style={{
                  ...
                  <div>
                    <div style={{
                      ...
                      fontSize: '0.75rem', color: categoryColor,
                      fontWeight: 'bold', marginBottom: '0.5rem'
                    }}>
                      {currentSecret.emoji} {currentSecret.category}
                    </div>
                    <h2 style={{
                      color: '#f0e6d0', fontSize: '1.25rem',
                      fontWeight: 'bold', margin: 0, lineHeight: '1.3'
                    }}>
                      {currentSecret.title}
                    </h2>
                  </div>
                  <div style={{
                    ...
                    fontSize: '1.5rem', flexShrink: 0, marginLeft: '1rem'
                  }}>
                    {currentSecret.emoji}
                  </div>
                </div>
                ...
                <div style={{ padding: '1.5rem' }}>
                  <p style={{
                    color: '#f0e6d0', fontSize: '1rem', lineHeight: '1.8',
                  ...
                  <div style={{
                    ...
                    <div style={{ color: categoryColor, fontSize: '0.8rem', fontWeight: 'bold', marginBottom: '0.4rem' }}>
                      📜 {currentSecret.verse}
                    </div>
                    <div style={{ color: '#f0e6d0', fontSize: '0.9rem', fontStyle: 'italic', lineHeight: '1.6' }}>
                  ...
                  {showDeepDive && (
                    <div style={{
                      ...
                      <div style={{ color: '#c8a96e', fontSize: '0.8rem', fontWeight: 'bold', marginBottom: '0.5rem' }}>
                        🔍 Deep Dive
                      </div>
                      <p style={{ color: '#f0e6d0', fontSize: '0.9rem', lineHeight: '1.7', margin: 0 }}>
                  ...
                  <div style={{ display: 'flex', gap: '0.6rem', flexWrap: 'wrap' }}>
                    <button onClick={() => setShowDeepDive(!showDeepDive)} style={{
                      background: showDeepDive ? `${categoryColor}30` : 'rgba(255,255,255,0.06)',
                      border: `1px solid ${categoryColor}40`,
                      borderRadius: '10px', padding: '0.6rem 1rem',
                      color: categoryColor, fontSize: '0.85rem',
                      cursor: 'pointer', fontFamily: "'Georgia', serif",
                      fontWeight: showDeepDive ? 'bold' : 'normal'
                    }}>
                      🔍 {showDeepDive ? 'Hide' : 'Deep Dive'}
                    </button>

                    <button onClick={toggleBookmark} style={{
                      background: isBookmarked ? 'rgba(240,192,64,0.2)' : 'rgba(255,255,255,0.06)',
                    ...
                    <button onClick={handleShare} style={{
                      background: copied ? 'rgba(160,216,160,0.2)' : 'rgba(255,255,255,0.06)',
                    ...
                    <button onClick={handleNext} style={{
                      ...
                      padding: '0.6rem 1.2rem', color: '#1a0a00',
                ...
                <div style={{
                  ...
                  <span style={{ color: 'rgba(200,169,110,0.5)', fontSize: '0.75rem' }}>
                    Secret #{currentSecret.id} of {totalSecrets}
                  </span>
                  <span style={{ color: 'rgba(200,169,110,0.5)', fontSize: '0.75rem' }}>
                    {viewed.includes(currentSecret.id) ? '👁 Seen before' : '✨ New discovery!'}
                  </span>
                </div>
              </div>
            )}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            {bookmarks.length === 0 ? (
              <div style={{
                ...
                background: 'rgba(255,255,255,0.04)',
                borderRadius: '16px', border: '1px solid rgba(240,192,64,0.15)'
              }}>
                <div style={{ fontSize: '3rem', marginBottom: '1rem' }}>🔖</div>
                <div style={{ color: '#c8a96e', fontSize: '1rem' }}>No saved secrets yet</div>
                <div style={{ color: 'rgba(200,169,110,0.5)', fontSize: '0.85rem', marginTop: '0.5rem' }}>
                  Tap "Save" on any secret to bookmark it here
                </div>
              </div>
            ) : (
                ...
                return (
                  <div key={secret.id} style={{
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.75rem' }}>
                      <div>
                        <div style={{ color, fontSize: '0.75rem', fontWeight: 'bold', marginBottom: '0.3rem' }}>
                      ...
                      <button
                        onClick={() => {
                          const newBookmarks = bookmarks.filter(b => b.id !== secret.id)
                          setBookmarks(newBookmarks)
                          localStorage.setItem('scriptureHub_bookmarks', JSON.stringify(newBookmarks))
                        }}
                        style={{
                    ...
                    <p style={{ color: '#c8a96e', fontSize: '0.88rem', lineHeight: '1.6', margin: '0 0 0.75rem 0' }}>
                      {secret.fact}
                    </p>
